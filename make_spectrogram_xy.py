...
# Output:
#       full screen plot of spectrograms from both the x and y axis
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
if ans1 == "pph":
    ...
    Pxx, freqs, bins, im = ax1.specgram(data_V_x, NFFT = int_step, noverlap = 0, Fs = sampling_frequency, xextent=(data[0,0], data[-1,0]), cmap='jet', vmin=-60, vmax=0)
    # --Pxx: the spectrogram
    ...
    Pxx, freqs, bins, im = ax2.specgram(data_V_y, NFFT = int_step, noverlap = 0, Fs = sampling_frequency, xextent=(data[0,0], data[-1,0]), cmap='jet', vmin=-60, vmax=0)
    # --Pxx: the spectrogram
    # --freqs: the frequency vector
    # --bins: the centers of the time bins
    # --im: the .image.AxesImage instance representing the data in the plot
    ...
    plt.colorbar(im,ax=(ax1,ax2), aspect=40, fraction=0.01, label="dB")
...
if ans1 == "rtd":
    ...
    while 1: 
        ...
        # plotting
        ax1 = plt.subplot(2,1,1)
        Pxx, freqs, bins, im = ax1.specgram(data_V_x, NFFT = int_step, noverlap = 0, Fs = sampling_frequency, xextent=(data_t[0], data_t[-1]), cmap='jet', vmin=-60, vmax=0)
        # --Pxx: the spectrogram
        # --freqs: the frequency vector
        # --bins: the centers of the time bins
        # --im: the .image.AxesImage instance representing the data in the plot
        ...
        ax2 = plt.subplot(2,1,2, sharex=ax1)
        Pxx, freqs, bins, im = ax2.specgram(data_V_y, NFFT = int_step, noverlap = 0, Fs = sampling_frequency, xextent=(data_t[0], data_t[-1]), cmap='jet', vmin=-60, vmax=0)
        # --Pxx: the spectrogram
        # --freqs: the frequency vector
        # --bins: the centers of the time bins
        # --im: the .image.AxesImage instance representing the data in the plot
